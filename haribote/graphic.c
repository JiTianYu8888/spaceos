...
{
	int i, eflags;
	eflags = io_load_eflags();	/* Š„‚èž‚Ý‹–‰Âƒtƒ‰ƒO‚Ì’l‚ð‹L˜^‚·‚??*/
	io_cli(); 					/* ‹–‰Âƒtƒ‰ƒO‚??‚É‚µ‚ÄŠ„‚èž‚Ý‹ÖŽ~‚É‚·‚??*/
...
void init_screen8(char *vram, int x, int y)
{
	...
	boxfill8(vram, x, COL8_000000,  3,     y - 24, 59,     y - 24);
	boxfill8(vram, x, COL8_000000,  2,     y - 24,  2,     y -  4);
	boxfill8(vram, x, COL8_FFFFFF,  3,     y -  4, 59,     y -  4);
	boxfill8(vram, x, COL8_FFFFFF, 59,     y - 23, 59,     y -  5);
	boxfill8(vram, x, COL8_000000,  2,     y -  3, 59,     y -  3);
	boxfill8(vram, x, COL8_000000, 60,     y - 24, 60,     y -  3);

	boxfill8(vram, x, COL8_000000, x - 47, y - 24, x -  4, y - 24);
	boxfill8(vram, x, COL8_000000, x - 47, y - 23, x - 47, y -  4);
	boxfill8(vram, x, COL8_000000, x - 47, y -  3, x -  4, y -  3);
	boxfill8(vram, x, COL8_000000, x -  3, y - 24, x -  3, y -  3);
	return;
}
		...
		for (; *s != 0x00; s++) {
			if (task->langbyte1 == 0) {
				if (0xa1 <= *s && *s <= 0xfe) {
					...
					putfont8(vram, xsize, x, y, c, hankaku + *s * 16);//Ö»ÒªÊÇ°EÇ¾ÍÊ¹ÓÃhankakuÀEæµÄ×Ö·E
				}
			} else {
				k = task->langbyte1 - 0xa1;
				t = *s - 0xa1;
