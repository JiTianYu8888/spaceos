...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
    ...
    /*else if (i == 3) { //如果定?器向?冲区?入的是3
    timeer++;//???行增加
    if (timeer == 24 * 60 * 60) {timeer = 0;}//??不能超?24:00:00
    miao = timeer % 60;//改??示的秒?部分
    fen = timeer / 60 % 60;//改??示的分?部分
    shi = timeer / (60 * 60) % 24;//改??示的??部分
    sprintf(mi, "%d", miao);//将整型数据???字符串?型来?出
    sprintf(fe, "%d", fen);
    sprintf(sh, "%d", shi);
    ...
    putfonts8_asc_sht(sht_back, binfo->scrnx - 20 - 10 - 20 - 10 - 20, 8, COL8_FFFFFF, COL8_008484, sh, 2);//?出
    timer_init(timer2, &fifo, 3); //定?器初始化，???行
	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	for (;;) {
             ...
             putfonts8_asc_sht(sht_back, binfo->scrnx - 130, binfo->scrny -20, COL8_000000, COL8_FFFFFF, s, 10);
			 sprintf(s, "%d:%d", get_hour_hex(), get_min_hex());
             putfonts8_asc_sht(sht_back, binfo->scrnx - 45, binfo->scrny -20, COL8_000000, COL8_FFFFFF, s, 5);
             sheet_refresh(sht_back, binfo->scrnx - 126, binfo->scrny -20,binfo->scrnx - 45 + 5*8, binfo->scrny -50+16);
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	make_window8(buf, 256, 165, "SpaceOS Terminal", 0);
